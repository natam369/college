	...
	public String inUniversityDao(int universityId, String universityName) throws DaoException {
		...
		try {
			if (con != null) {
				 ...
				 * PreparedStatement p = con.prepareStatement(sql); p.setInt(1, universityId);
				 * p.setString(2, universityName); p.executeUpdate();
				 */
				String query = "{call insertuniversity(?,?)}";
				CallableStatement cs = con.prepareCall(query);
				cs.setInt(1, universityId);
				cs.setString(2, universityName);
				cs.executeUpdate();
...
//				ResultSet rs = p.executeQuery();
				String query = "{call getalluniversityids()}";
				CallableStatement cs = con.prepareCall(query);
	...
	public ArrayList<Details> getDetailsDao(int uid) throws DaoException {
		...
		try {
			...
			if (con != null) {
				/*
				 * String sql =
				 * "select college.cid,college.cname,college.rating,university.uid,university.uname from college inner join university on college.uid=university.uid where university.uid=?"
				 * ; PreparedStatement p = con.prepareStatement(sql); p.setInt(1, uid);
				 */
	...
	public ArrayList<Details> getDetailsDao(double rating) throws DaoException {
		...
		try {
			...
			if (con != null) {
				// String sql = "select
				// college.cid,college.cname,college.rating,university.uid,university.uname from
				// college inner join university on college.uid=university.uid where
				// college.rating>?";
				// p = con.prepareStatement(sql);
				// p.setDouble(1, rating);
				// ResultSet rs = p.executeQuery();
				String query = "{call getdetailsbyrating(?)}";
				System.out.println("hai");
				cs = con.prepareCall(query);
				System.out.println("successexecute");
				cs.setDouble(1, rating);
